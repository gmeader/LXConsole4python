...
#     refer to READ ME.txt for configuration information
#
#################################################################
...
class App:
    ...
    def __init__(self, master):
        ...
        windowmenu.add_command(label='Cues', command=self.menuCueList)
        windowmenu.add_command(label='Patch', command=self.menuPatchList)
    ...
    def set_usb_out(self):
        if self.cues.livecue.output is not None:
    ...
    def menuSave(self):
        if len(self.path) > 0:
            filename = tkfile_dialog.asksaveasfilename(defaultextension="asc", initialfile=os.path.basename(self.path),
    ...
    def menuCueList(self):
        self.displayCues()

    def menuPatchList(self):
        self.displayPatch()

    def menuAbout(self):
        tkmsg_box.showinfo(message='LXConsole|Python v 0.8',
                           detail='build 4026\nCopyright 2015-2018 Claude Heintz Design\nSee source files for license info.',
                           icon='info', title='About LXConsole')
    ...
    def fadeComplete(self):
        if not self.cues.livecue.stopped:
    ...
    def updateCurrent(self):
        if self.cues.current is not None:
